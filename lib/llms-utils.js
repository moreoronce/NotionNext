...
const getLatestContentDate = pages => {
  const latest = pages
    .map(getPostPriorityDate)
...
export const createLlmsTxt = ({
  ...
  const title = trimText(
    siteInfo?.title || siteConfig('TITLE', BLOG.TITLE, NOTION_CONFIG),
    120
  )
  ...
  const postLimit =
    Number(siteConfig('LLMS_TXT_POST_LIMIT', 80, NOTION_CONFIG)) || 80
  ...
  const lastUpdated = getLatestContentDate(allPages.filter(isPublicContentPage))
